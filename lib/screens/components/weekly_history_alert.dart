...
import 'metro_stamp_20_anniversary_info_display_alert.dart';
...
class _WeekHeaderState extends ConsumerState<WeekHeader> with ControllersMixin<WeekHeader> {
  ...
  ///
  Widget displayWeeklyHistoryStampRallyWidget({required String date}) {
    if (appParamState.keepDateMetroStampMap[date] == null &&
        appParamState.keepMetroStamp20AnniversaryMap[date] == null) {
      return const SizedBox.shrink();
    }
      ...
      onTap: () {
        LifetimeDialog(
          ...
          widget: (appParamState.keepDateMetroStampMap[date] != null)
              ...
              : MetroStamp20AnniversaryInfoDisplayAlert(date: date),
        );
      },
    );
  }
